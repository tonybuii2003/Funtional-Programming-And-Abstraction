...
public class HigherOrderUtils {
     ...
     *                    applied.
     * @param bifunctions the list of bifunctions that will be applied on <ocde>
     *                    args</code>.
     * @param <T>         the type parameter of the arguments (e.g., Integer,
     *                    double).
     * @return of all the bifunctions being applied in seqeunce.
     *
     *
    ...
    public static <T> T zip(List<T> args, List<? extends BiFunction<T, T, T>> bifunctions) {
        if (bifunctions.size() + 1 != args.size())
            throw new IllegalArgumentException(
                    "The number of bifunction elements and the number of argument elements do not match up as required.");
        Iterator<? extends BiFunction<T, T, T>> bifunctionIterator = bifunctions.iterator();
        T tmp = args.get(0);
        for (int i = 1; i < args.size(); i++) {
            tmp = bifunctionIterator.next().apply(tmp, args.get(i));
            args.set(i, tmp);
        }
